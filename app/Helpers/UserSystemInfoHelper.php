...
class UserSystemInfoHelper
{
	...
	public static function get_device(){
		...
		if(!empty($_SERVER['HTTP_USER_AGENT']) && preg_match('/(tablet|ipad|playbook)|(android(?!.*(mobi|opera mini)))/i', strtolower($_SERVER['HTTP_USER_AGENT']))){
			$tablet_browser++;
		}

		if(!empty($_SERVER['HTTP_USER_AGENT']) &&  preg_match('/(up.browser|up.link|mmp|symbian|smartphone|midp|wap|phone|android|iemobile)/i', strtolower($_SERVER['HTTP_USER_AGENT']))){
			$mobile_browser++;
		}

		if(!empty($_SERVER['HTTP_ACCEPT']) && (strpos(strtolower($_SERVER['HTTP_ACCEPT']),
